(function($) {
	...
	$(document).ready(function() {
		...
		var table = $("#shipments").DataTable({
			...
			columns: [
				{
					...
					render: function(data, type, row) {
						...
						if (type === "display") {
							if (
								(rowData.qb_rt == 1 && rowData.qb_ws == 1) ||
								rowData.is_complete == 1 ||
								rowData.rcvd == 1
							) {
							...
							return (
								'<div id="status_div" class = "' + color + '"><p></p></div>'
							);
						}
						return data;
					}
			...
			createdRow: function(row, data, dataIndex) {
				...
				if (data.shipments.has_documents == 1) {
					$("td:last-child", row).html(
						'<a href="/richfilemanager?expandedFolder=' +
							data.shipments.file_directory +
							'" target="_blank"><img src="/assets/folder_icon.png"></a>'
					);
				} else {
					$("td:last-child", row).html(
						'<img src="/assets/folder_icon.png" class="disabled_folder_icon">'
					);
				}
			},
			rowCallback: function(row, data, displayNum, displayIndex, dataIndex) {
				if (
					...
					data.shipments.rcvd == 1 ||
					data.shipments.latest_event.includes("Empty Container Returned")
				) {
				...
				if (
					...
					$("td:nth-child(1)", row).html(
						'<div id="status_div" class = "circle_disabled"></div>'
					);
					/*table.row(row)
                            .data(data)
                            .draw();*/
				...
				$("#editor-isfrequired", row).prop(
					"checked",
					data.shipments.isf_required == 1
				);
				$("#editor-customs", row).prop("checked", data.shipments.customs == 1);
				$("#editor-po_boolean", row).prop(
					"checked",
					data.shipments.po_boolean == 1
				);
				$("#editor-qb_rt", row).prop("checked", data.shipments.qb_rt == 1);
				$("#editor-qb_ws", row).prop("checked", data.shipments.qb_ws == 1);
				$("#editor-requires_payment", row).prop(
					"checked",
					data.shipments.requires_payment == 1
				);
				...
				if (
					(data.shipments.qb_ws == 1 && data.shipments.qb_rt == 1) ||
					data.shipments.rcvd == 1
				) {
					$("td:first-child > div#status_div", row).removeClass("circle_green");
					$("td:first-child > div#status_div", row).removeClass(
						"circle_yellow"
					);
					...
					if (data.shipments.eta === null) {
						...
						daysDifference = datediff(
							parseDate(today),
							parseDate(data.shipments.eta)
						);
					}
					if (data.shipments.truck_date === null) {
						daysDifferenceTruck = -1;
					} else {
						daysDifferenceTruck = datediff(
							parseDate(today),
							parseDate(data.shipments.truck_date)
						);
					}
					...
					if (daysDifference > 7) statusTranslation = "circle_green";
						...
						statusTranslation = "circle_yellow";
		...
		$("#shipments")
			.on("change", "#editor-rcvd", function() {
				editor
				...
				$(this)
					.closest("td")
					.removeClass($(this).prop("checked") ? "status_red" : "status_blue");
				$(this)
					.closest("td")
					.addClass($(this).prop("checked") ? "status_blue" : "status_red");
			...
			.on("change", "#editor-po_boolean", function() {
				editor
					.edit($(this).closest("tr"), false, {
						submit: "changed"
					})
