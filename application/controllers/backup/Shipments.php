        ...
        */
        ...
        foreach($usefulData as $somethingWorthwhile){
            ...
            if (ctype_digit($testVal) && strlen($testVal)===6){
                ...
                $container = json_decode(json_encode($this->ShipmentsModel->get_by_container_number($somethingWorthwhile[0].$somethingWorthwhile[1])));
                $lfd=null;
                if (!is_null($container->lfd) && !empty($container->lfd)) $lfd = new DateTime($container->lfd);
                if (is_null($lfd) || (!is_null($lfd) && $etaStrToTime < $lfd)){
                    $this->ShipmentsModel->update_record(array('container_number' => $somethingWorthwhile[0].substr($somethingWorthwhile[1],0,6)),
