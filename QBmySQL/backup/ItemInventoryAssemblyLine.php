...
$TRUNCATE_Result = mysqli_query($link, "TRUNCATE TABLE ItemInventoryAssemblyLine"); // insert one row into new table
if ($TRUNCATE_Result) {
    echo "Success";
...
foreach($rows as $row) {
   ...
   print("<br>-----------------------------<br>");
   $res=mysqli_query($link,"INSERT INTO ItemInventoryAssemblyLine (".implode(", ",$keys).") VALUES ('".implode("', '",$values)."')"); // insert one row into new table
   if($res)
	{
	echo "Success";
	...
	{
	print("INSERT INTO ItemInventoryAssemblyLine (".implode(", ",$keys).") VALUES ('".implode("', '",$values)."')");
