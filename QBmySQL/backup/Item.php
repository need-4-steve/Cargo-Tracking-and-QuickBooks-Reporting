...
$TRUNCATE_Result = mysqli_query($link, "TRUNCATE TABLE Item"); // insert one row into new table
if ($TRUNCATE_Result) {
    echo "Success";
...
foreach ($rows as $row) {
    ...
    print("<br>-----------------------------<br>");
    $res = mysqli_query($link, "INSERT INTO item (" . implode(", ", $keys) . ") VALUES ('" . implode("', '", $values) . "')"); // insert one row into new table
    if ($res) {
        echo "Success";
        
    } else {
        print("INSERT INTO item (" . implode(", ", $keys) . ") VALUES ('" . implode("', '", $values) . "')");
